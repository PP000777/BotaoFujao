let topMod = 0;
        ...
        function mobileHandler() {
            // Incrementa o estado e volta ao início se exceder 4
            mobileStates = (mobileStates % 4) + 1;

            // Atualiza topMod e leftMod com base no estado atual
            topMod += mobile_states[mobileStates].top * 10;
            leftMod += mobile_states[mobileStates].left * 10;

            // Aplica as novas posições ao botão #yes
            $("#yes").css({ top: topMod + 'px', left: leftMod + 'px', position: 'relative' });
        }

        if (window.mobileCheck()) {
            $("#yes").mouseover(mobileHandler);
        } else {
        ...
        function webHandler(event) {
            ...
            if (distancia < 80) {
                let mods = calculateAngle(event, button, distancia);
